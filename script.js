...
if (settingAnimations) {
  settingAnimations.addEventListener('change', () => {
    ...
    applyVisualSettings();
const langLabels = typeof langLabels === 'undefined' ? { en: 'EN', sq: 'SHQ', de: 'DE', fr: 'FR', ru: 'RU' } : langLabels;
...
function buildControllerHints() {
  if (document.getElementById('controller-hints')) return;
  const bar = document.createElement('div');
  bar.id = 'controller-hints';
  bar.className = 'controller-hints';
  bar.innerHTML = `
    <span class="controller-hint"><span class="btn">A/✕</span> Select</span>
    <span class="controller-hint"><span class="btn">B/○</span> Back</span>
    <span class="controller-hint"><span class="btn">X/□</span> Legend</span>
    <span class="controller-hint"><span class="btn">Y/△</span> Compare</span>
    <span class="controller-hint"><span class="btn">LB/RB</span> Theme</span>
  `;
  document.body.appendChild(bar);
}

function setControllerActive(active) {
  controllerState.connected = active;
  document.body.classList.toggle('controller-active', active);
  const bar = document.getElementById('controller-hints');
  if (bar) bar.classList.toggle('active', active);
}

function getVisibleTiles() {
  return Array.from(document.querySelectorAll('.element[data-number]')).filter((tile) => {
    if (!tile.dataset.number) return false;
    if (tile.style.display === 'none') return false;
    return tile.offsetParent !== null;
  });
}
...
function setGamepadFocus(index) {
  ...
  tiles.forEach((tile) => tile.classList.remove('gamepad-focus'));
  const tile = tiles[clamped];
  tile.classList.add('gamepad-focus');
  tile.scrollIntoView({ block: 'nearest', inline: 'nearest' });
  controllerState.index = clamped;
}

function ensureGamepadFocus() {
  const tiles = getVisibleTiles();
  if (!tiles.length) return;
  const selectedIdx = selectedNumber
    ? tiles.findIndex((t) => Number(t.dataset.number) === selectedNumber)
    : -1;
  if (selectedIdx >= 0) {
    setGamepadFocus(selectedIdx);
    return;
  }
  setGamepadFocus(controllerState.index || 0);
}

function moveGamepadFocus(delta) {
  const tiles = getVisibleTiles();
  if (!tiles.length) return;
  if (controllerState.index == null) controllerState.index = 0;
  const next = controllerState.index + delta;
  setGamepadFocus(next);
}
...
function cycleTheme(dir) {
  const themes = ['focus', 'light', 'sunset', 'ocean', 'forest'];
  const idx = Math.max(0, themes.indexOf(currentTheme));
  const next = (idx + dir + themes.length) % themes.length;
  applyTheme(themes[next]);
}

function pollGamepad() {
  if (!controllerState.connected || controllerState.gamepadIndex === null) return;
  const gp = navigator.getGamepads()[controllerState.gamepadIndex];
  if (!gp) return;

  const now = Date.now();
  const pressed = (idx) => gp.buttons[idx] && gp.buttons[idx].pressed;
  const justPressed = (idx) => {
    const was = controllerState.lastButtons[idx];
    const is = pressed(idx);
    controllerState.lastButtons[idx] = is;
    return is && !was;
  };

  const axisX = gp.axes[0] || 0;
  const axisY = gp.axes[1] || 0;
  const canRepeat = now - controllerState.lastMove > 180;

  if (justPressed(15) || (axisX > 0.6 && canRepeat)) {
    moveGamepadFocus(1);
    controllerState.lastMove = now;
  }
  if (justPressed(14) || (axisX < -0.6 && canRepeat)) {
    moveGamepadFocus(-1);
    controllerState.lastMove = now;
  }
  if (justPressed(12) || (axisY < -0.6 && canRepeat)) {
    moveGamepadFocus(-18);
    controllerState.lastMove = now;
  }
  if (justPressed(13) || (axisY > 0.6 && canRepeat)) {
    moveGamepadFocus(18);
    controllerState.lastMove = now;
  }
  ...
  if (justPressed(0)) {
    ensureGamepadFocus();
    const tiles = getVisibleTiles();
    const tile = tiles[controllerState.index];
    if (tile && tile.dataset.number) {
      selectElement(Number(tile.dataset.number));
    }
  }
  ...
  if (justPressed(2) && toggleLegend) toggleLegend.click();
  if (justPressed(3) && toggleCompare) toggleCompare.click();
  if (justPressed(4)) cycleTheme(-1);
  if (justPressed(5)) cycleTheme(1);

  requestAnimationFrame(pollGamepad);
}

window.addEventListener('gamepadconnected', (event) => {
  controllerState.gamepadIndex = event.gamepad.index;
...
window.addEventListener('gamepaddisconnected', (event) => {
  if (controllerState.gamepadIndex === event.gamepad.index) {
    controllerState.gamepadIndex = null;
    setControllerActive(false);
    document.querySelectorAll('.gamepad-focus').forEach((tile) => tile.classList.remove('gamepad-focus'));
  }
});
  });
}
